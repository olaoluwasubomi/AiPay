...
import Loader from "@/components/Loader"; // ✅ Reusable loading overlay
    ...
    <Layout>
      <div className="flex items-center justify-between">
        ...
        <div className="space-x-2">
          <button onClick={onMarkAll} className="px-4 py-2 border rounded-lg">
            Mark all as read
          </button>
          <button onClick={onClearAll} className="px-4 py-2 border rounded-lg">
            Clear all
          </button>
        </div>
      </div>

      <div className="bg-white rounded-xl shadow mt-6 relative">
        ...
        {loading ? (
          ...
          <>
            {items.length > 0 ? (
              ...
              <div className="px-5 py-8 text-gray-500">No notifications yet</div>
            )}
          </>
        )}
      </div>
